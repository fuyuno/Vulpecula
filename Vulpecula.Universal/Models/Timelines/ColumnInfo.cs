...
namespace Vulpecula.Universal.Models.Timelines
{
    public class ColumnInfo : BindableBase
    {
        ...
        private ColumnInfo(TimelineType type, string id, string name, long userId, string query)
        {
            this.Type = type;
            this.ColumnId = id;
            this.Name = name;
        ...
        public static ColumnInfo CreateColumnInfo(TimelineType type, string name, long userId, string query = null)
        {
            return new ColumnInfo(type, $"Column-{Guid.NewGuid()}", name, userId, query);
        }

        public static ColumnInfo RestoreColumnInfo(ApplicationDataCompositeValue adcv)
        {
            TimelineType type;
